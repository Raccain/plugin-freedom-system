...
next_action: improve_drum_detection
next_phase: improvement
current_version: v1.1.0β
contract_checksums:
...
## Current State: v1.1.0 — Key Detection Fixed

v1.1.0 improves key detection with 3 fixes: confidence threshold, 4th-order HP filter, bin amplitude floor.
Windows build now works (WebView2 enabled). Next: drum pattern detection improvements.
...
- v1.0.5β — bugfixes: wrong stop button (analyzeTriggered not reset on new recording), stop-on-close (editor destructor now stops capture/preview), real-time freq filtering during audition (IIR bandpass applied in processBlock per selected band) (Feb 23)
- v1.0.6β — onset detection: band-specific min inter-onset gaps (kick=80ms, snare=60ms, hihat=30ms), absolute strength floor (12% of peak) to suppress false positives (Feb 23)
- v1.0.7β — sensitivity gate in audition: Sens slider now gates preview audio (instant attack, ~75ms release, threshold=peak*(1-sens)*0.35), fast-open/slow-close to preserve transient attack and hit tails (Feb 23)
- v1.0.8β — (unstaged changes present, version bumped but no recorded change)
- v1.0.9β — precision fix: strength floor now sensitivity-controlled (sens=0 → 55% of peak, sens=1 → 5%); adaptive threshold multiplier raised 4×→6× so low sensitivity only captures dominant transients (Feb 23)
- v1.1.0β — key detection fix: 3 improvements — confidence threshold (corr < 0.5 → "Unknown"), 4th-order Butterworth HP at 150 Hz (was 1st-order 100 Hz), per-frame bin amplitude floor (skip < 1% of peak). Windows build enabled (JUCE_USE_WIN_WEBVIEW2=1 + WebView2 SDK). (Feb 23)
...
- [x] Duration auto-stop — set 5s, play 6s, UI should transition to READY without manual STOP click (FIXED in v1.0.4β — knob now wires to C++ APVTS) (VERIFIED)
- [x] Buffer status after stop — shows "READY • 12.4s", persists through analyze/complete states (FIXED in v1.0.4β) (VERIFIED)
- [x] Analysis completes and shows real BPM/key — REQUIRES real audio (not silence) (VERIFIED)
- [x] MIDI tiles show ONLY clips that were actually detected (VERIFIED)
- [x] Audition plays back recorded audio — REQUIRES real audio in buffer (VERIFIED) 
- [x] MIDI drag-to-DAW — drag tiles to Ableton/Logic/Reaper MIDI track (FIXED in v1.0.4β — passes this as source component) (VERIFIED)
...
## Next Steps (v1.2 — Drum Detection Improvements)

Use `/improve GrooveScout` to continue improvements. Planned drum detection upgrades:

**Phase 1 (High priority):** Spectral flux onset detection + 4th-order Butterworth bandpass filters
**Phase 2 (Medium):** Beat-grid quantization (snap to 1/16th, reject false positives)
**Phase 3 (If needed):** HPSS preprocessing (harmonic/percussive separation)

Other feature ideas from user:
- Prelisten MIDI clips with matched samples + metronome
- Advanced Setup per instrument (attack/release gate controls)
- Beat alignment / buffer trim for precise 1-2-3-4 positioning
